...
public class Patch {
	...
	public static void processPlayerPatch(NetHandlerPlayServer net,
		...
		WorldServer worldserver = ((MinecraftServer) ObfuscationReflectionHelper
				.getPrivateValue(NetHandlerPlayServer.class, net, 2))
				.worldServerForDimension(net.playerEntity.dimension);
		...
		if (!net.playerEntity.playerConqueredTheEnd) {
			...
			if (!((Boolean) ObfuscationReflectionHelper.getPrivateValue(
				...
				d0 = packet.func_149467_d()
						- ((Double) ObfuscationReflectionHelper
								.getPrivateValue(NetHandlerPlayServer.class,
										net, 15));

				if (packet.func_149464_c() == ((Double) ObfuscationReflectionHelper
						.getPrivateValue(NetHandlerPlayServer.class, net, 14))
						&& d0 * d0 < 0.01D
						&& packet.func_149472_e() == ((Double) ObfuscationReflectionHelper
								.getPrivateValue(NetHandlerPlayServer.class,
										net, 16))) {
					ObfuscationReflectionHelper.setPrivateValue(
							NetHandlerPlayServer.class, net, true, 17);
				}
			}
			...
			if (((Boolean) ObfuscationReflectionHelper.getPrivateValue(
				...
				if (net.playerEntity.ridingEntity != null) {
					...
					((MinecraftServer) ObfuscationReflectionHelper
							...
							.updatePlayerPertinentChunks(net.playerEntity);

					if (((Boolean) ObfuscationReflectionHelper.getPrivateValue(
							NetHandlerPlayServer.class, net, 17))) {
						ObfuscationReflectionHelper.setPrivateValue(
								NetHandlerPlayServer.class, net,
								net.playerEntity.posX, 14);
						ObfuscationReflectionHelper.setPrivateValue(
								NetHandlerPlayServer.class, net,
								net.playerEntity.posY, 15);
						ObfuscationReflectionHelper.setPrivateValue(
								NetHandlerPlayServer.class, net,
								net.playerEntity.posZ, 16);
					}
				...
				ObfuscationReflectionHelper.setPrivateValue(
						NetHandlerPlayServer.class, net, net.playerEntity.posX,
						14);
				ObfuscationReflectionHelper.setPrivateValue(
						NetHandlerPlayServer.class, net, net.playerEntity.posY,
						15);
				ObfuscationReflectionHelper.setPrivateValue(
						NetHandlerPlayServer.class, net, net.playerEntity.posZ,
						16);
				d1 = net.playerEntity.posX;
				d2 = net.playerEntity.posY;
				d3 = net.playerEntity.posZ;
				...
				double d7 = Math.max(Math.abs(d4),
						Math.abs(net.playerEntity.motionX));
				double d8 = Math.max(Math.abs(d5),
						Math.abs(net.playerEntity.motionY));
				double d9 = Math.max(Math.abs(d6),
						Math.abs(net.playerEntity.motionZ));
				...
				if (d10 > 0.0625D && !net.playerEntity.isPlayerSleeping()
						&& !net.playerEntity.theItemInWorldManager.isCreative()) {
					flag1 = true;
					((Logger) ObfuscationReflectionHelper.getPrivateValue(
							NetHandlerPlayServer.class, net, 0))
							.warn(net.playerEntity.getCommandSenderName()
				...
				if (flag
						&& (flag1 || !flag2)
						&& !net.playerEntity.isPlayerSleeping()
						&& !net.playerEntity.noClip
				...
				AxisAlignedBB axisalignedbb = net.playerEntity.boundingBox
						.copy().expand((double) f3, (double) f3, (double) f3)
						.addCoord(0.0D, -0.55D, 0.0D);

				if (!((MinecraftServer) ObfuscationReflectionHelper
						.getPrivateValue(NetHandlerPlayServer.class, net, 2))
						.isFlightAllowed()
						&& !net.playerEntity.theItemInWorldManager.isCreative()
						&& !worldserver.checkBlockCollision(axisalignedbb)
						&& !net.playerEntity.capabilities.allowFlying) {
					if (d11 >= -0.03125D) {
						ObfuscationReflectionHelper.setPrivateValue(
								NetHandlerPlayServer.class, net,
								((Integer) ObfuscationReflectionHelper
										.getPrivateValue(
												NetHandlerPlayServer.class,
												net, 5)) + 1, 5);

						if (((Integer) ObfuscationReflectionHelper
								.getPrivateValue(NetHandlerPlayServer.class,
										net, 5)) > 80) {
					...
					ObfuscationReflectionHelper.setPrivateValue(
							NetHandlerPlayServer.class, net, 0, 5);
				}
				...
				net.playerEntity.onGround = packet.func_149465_i();
				((MinecraftServer) ObfuscationReflectionHelper.getPrivateValue(
						NetHandlerPlayServer.class, net, 2))
						.getConfigurationManager().updatePlayerPertinentChunks(
								net.playerEntity);
				net.playerEntity.handleFalling(net.playerEntity.posY - d0,
						packet.func_149465_i());
			} else if (((Integer) ObfuscationReflectionHelper.getPrivateValue(
					NetHandlerPlayServer.class, net, 4)) % 20 == 0) {
				net.setPlayerLocation(((Double) ObfuscationReflectionHelper
						.getPrivateValue(NetHandlerPlayServer.class, net, 14)),
						((Double) ObfuscationReflectionHelper.getPrivateValue(
								NetHandlerPlayServer.class, net, 15)),
